                ...
                <div class="reminders bg-warning text-white p-3 rounded">
                    ...
                    <?php if (!empty($reminders)) : ?>
                        <?php foreach ($reminders as $reminder) : ?>
                            <div class="reminder-item bg-light text-dark p-2 mb-2 rounded" data-event-id="<?= $reminder['event_id'] ?>">
...
function showDayEvents(event) {
    ...
    // Get the first valid date from events or reminders
    let clickedDate;
    const firstEvent = eventsAndReminders.find(item => item.start_date || (item[1] && item[1].notified_at));
    if (firstEvent) {
        clickedDate = new Date(firstEvent.start_date || firstEvent[1].notified_at);
    } else {
    ...
    const events = eventsAndReminders.filter(item => item.event_name && !item[1]);
    const reminders = eventsAndReminders.filter(item => item[1] || item.notification_id);
    ...
    if (reminders.length > 0) {
        ...
        reminders.forEach(reminder => {
            const reminderData = reminder[1] || reminder;
            const reminderTime = new Date(reminderData.notified_at);
            
            htmlContent += `
                <div class="mb-2 p-2 border rounded">
                    <strong>${reminderData.event_name}</strong>
                    <span class="badge bg-warning ms-2">Reminder</span>
                    <p class="mb-0 mt-1">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function attachEditListeners() {
        document.querySelectorAll('.edit-reminder-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const notifiedAt = this.getAttribute('data-notified-at');
                
                let reminderTime = '';
                
                if (notifiedAt) {
                    reminderTime = new Date(notifiedAt).toISOString().slice(0, 16);
                } else if (notifications[eventId]?.notified_at) {
                    reminderTime = new Date(notifications[eventId].notified_at).toISOString().slice(0, 16);
                }
                
                const notes = notifications[eventId]?.notes || '';

                // Populate modal fields
                document.getElementById('reminder_event_id').value = eventId;
                document.getElementById('event_name').value = eventName;
                document.getElementById('notified_at').value = reminderTime;
    ...
    // Reattach listeners after calendar updates
...
// Handle form submission with immediate UI update
document.getElementById('reminderForm').addEventListener('submit', function(e) {
    e.preventDefault();
    
    const formData = new FormData(this);
    const eventId = formData.get('reminder_event_id');
    const notifiedAt = formData.get('notified_at');
    const notes = formData.get('notes');
    
    // Update notifications data structure
    if (!notifications[eventId]) {
        notifications[eventId] = {};
    }
    notifications[eventId].notified_at = notifiedAt;
    notifications[eventId].notes = notes;
    
    // Update calendar display
    updateCalendar();
    
    // Update data attributes
    const editButton = document.querySelector(`.edit-reminder-btn[data-event-id="${eventId}"]`);
    if (editButton) {
        editButton.setAttribute('data-notified-at', notifiedAt);
    }
